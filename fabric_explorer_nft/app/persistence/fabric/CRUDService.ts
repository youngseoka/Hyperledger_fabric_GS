...
export class CRUDService {
	...
	async saveTransaction_lowtech(network_name, transaction) {
		const c = await this.sql.getRowByPkOne(
			'select count(1) as c from transactions_lowtech where blockid=$1 and txhash=$2 and channel_genesis_hash=$3 and network_name = $4 ',
			[
				transaction.blockid,
				transaction.txhash,
				transaction.channel_genesis_hash,
		...
		if (isValidRow(c)) {
			transaction.network_name = network_name;
			await this.sql.saveRow('transactions_lowtech', transaction);
			await this.sql.updateBySql(
				'update chaincodes set txcount =txcount+1 where channel_genesis_hash=$1 and network_name = $2 and name=$3',
				[transaction.channel_genesis_hash, network_name, transaction.chaincodename]
			);
	...
	async saveTransaction_nipanft(network_name, transaction) {
		const c = await this.sql.getRowByPkOne(
			'select count(1) as c from transactions_nipanft where blockid=$1 and txhash=$2 and channel_genesis_hash=$3 and network_name = $4 ',
			[
				transaction.blockid,
				transaction.txhash,
				transaction.channel_genesis_hash,
		...
		if (isValidRow(c)) {
			transaction.network_name = network_name;
			await this.sql.saveRow('transactions_nipanft', transaction);
			await this.sql.updateBySql(
				'update chaincodes set txcount =txcount+1 where channel_genesis_hash=$1 and network_name = $2 and name=$3',
				[transaction.channel_genesis_hash, network_name, transaction.chaincodename]
			);
