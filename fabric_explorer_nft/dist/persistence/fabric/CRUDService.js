...
class CRUDService {
     ...
     * @memberof CRUDService
     */
    getBlockAndTxList(network_name, channel_genesis_hash, blockNum, from, to, orgs) {
    ...
    saveTransaction_lowtech(network_name, transaction) {
        return __awaiter(this, void 0, void 0, function* () {
            const c = yield this.sql.getRowByPkOne('select count(1) as c from transactions_lowtech where blockid=$1 and txhash=$2 and channel_genesis_hash=$3 and network_name = $4 ', [
                transaction.blockid,
                transaction.txhash,
                transaction.channel_genesis_hash,
                network_name
            ...
            if (isValidRow(c)) {
                transaction.network_name = network_name;
                yield this.sql.saveRow('transactions_lowtech', transaction);
                yield this.sql.updateBySql('update chaincodes set txcount =txcount+1 where channel_genesis_hash=$1 and network_name = $2 and name=$3', [transaction.channel_genesis_hash, network_name, transaction.chaincodename]);
                yield this.sql.updateBySql('update channel set trans =trans+1 where channel_genesis_hash=$1 and network_name = $2 ', [transaction.channel_genesis_hash, network_name]);
                return true;
            }
    ...
    saveTransaction_nipanft(network_name, transaction) {
        return __awaiter(this, void 0, void 0, function* () {
            const c = yield this.sql.getRowByPkOne('select count(1) as c from transactions_nipanft where blockid=$1 and txhash=$2 and channel_genesis_hash=$3 and network_name = $4 ', [
                transaction.blockid,
                transaction.txhash,
                transaction.channel_genesis_hash,
                network_name
            ...
            if (isValidRow(c)) {
                transaction.network_name = network_name;
                yield this.sql.saveRow('transactions_nipanft', transaction);
                yield this.sql.updateBySql('update chaincodes set txcount =txcount+1 where channel_genesis_hash=$1 and network_name = $2 and name=$3', [transaction.channel_genesis_hash, network_name, transaction.chaincodename]);
                yield this.sql.updateBySql('update channel set trans =trans+1 where channel_genesis_hash=$1 and network_name = $2 ', [transaction.channel_genesis_hash, network_name]);
                return true;
            }
