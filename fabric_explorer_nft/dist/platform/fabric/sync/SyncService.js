...
class SyncServices {
    ...
    processBlockEvent(client, block, noDiscovery) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let channel_name_youngseok = channel_name;
            ...
            for (let txIndex = 0; txIndex < txLen; txIndex++) {
                ...
                var kordate = new Date(txObj.payload.header.channel_header.timestamp);
                txObj.payload.header.channel_header.timestamp = kordate;
                var fcn = "";
                var nft_id = "";
                var balance = "";
                var timestamp = "";
                ...
                var nft_num = "";
                var nft_status = "";
                console.log("writeSet['1']");
                console.log(writeSet['1']);
                console.log("writeSet['1'].chaincode");
                console.log(writeSet['1'].chaincode);
                console.log("writeSet['1'].set[1].value.fcn");
                console.log(writeSet['1'].set[1].value.fcn);
                console.log("start");
                console.log(txObj.payload.header.channel_header.timestamp);
                //var kordate = new Date(txObj.payload.header.channel_header.timestamp);
                //kordate.setHours(kordate.getHours()+9);
                //txObj.payload.header.channel_header.timestamp = kordate;
                if (writeSet['1'].chaincode == "save_ipfs_hash") {
                    const key = JSON.parse(writeSet['1'].set[1].value);
                    if (key.fcn == 'NFT_CREATE') {
                        nft_id = key.nft_id;
                        balance = key.balance;
                        ...
                        title = key.title;
                        nft_num = key.nft_num;
                        nft_status = key.nft_status;
                    }
                    else if (key.fcn == 'Read_NFT') {
                        nft_id = key.nft_id;
                        fcn = key.fcn;
                        timestamp = key.timestamp;
                    }
                    else if (key.fcn == 'Burn_NFT') {
                        nft_id = key.nft_id;
                        fcn = key.fcn;
                        nft_status = key.nft_status;
                        timestamp = key.timestamp;
                ...
                //		if(writeSet['1'].chaincode=="interval_random")
                //		continue;
                ...
                if (channel_name_youngseok == 'digitalzonelowtech') {
                    const res_lowtech = yield this.persistence
                        .getCrudService()
                        .saveTransaction_lowtech(network_id, transaction_row);
                    logger.debug('saveTransaction ', res_lowtech);
                }
                else if (channel_name_youngseok == 'digitalzonenipanft') {
                    const res_nipa_nft = yield this.persistence
                        .getCrudService()
                        .saveTransaction_nipanft(network_id, transaction_row);
                    logger.debug('saveTransaction ', res_nipa_nft);
                }
            }
            // Insert block
