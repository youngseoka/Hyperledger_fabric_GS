...
const logger = helper_1.helper.getLogger('PgService');
/**
 *
 *
 * @class PgService
...
class PgService {
    ...
    constructor(pgconfig) {
        ...
        if (isPostgresSslEnabled) {
            const dbCertsPath = process.env.DATABASE_CERTS_PATH ||
    ...
    getUserModel(attributes, options) {
        const sequelize = new sequelize_1.Sequelize(`postgres://${this.pgconfig.user}:${this.pgconfig.password}@${this.pgconfig.host}:${this.pgconfig.port}/${this.pgconfig.database}`, { logging: false });
     ...
     *
     */
    getRowsBySQlNoCondition(sqlcharacter, values, limit) {
        /* eslint-disable */
        return new Promise((resolve, reject) => {
            let sql;
